...
inline double dist(Point p1,Point p2){
    return eps + sqrt((p1.x-p2.x)*(p1.x-p2.x)+(p1.y-p2.y)*(p1.y-p2.y));
}
//求点到线段距离
...
int main(){
    ...
    while(scanf("%d",&n)!=EOF&&n){
        ...
        double step=eps + sqrt((maxx-minx)*(maxx-minx)+(maxy-miny)*(maxy-miny))/2;
        bool flag=false;
        while(step>eps && !flag){
            for(int i=0;i<m&&!flag;i++){
                for(int j=0;j<5&&!flag;j++){
                    ...
                    Get_Min_Dist(cur);
                    if(cur.val+eps>tp[i].val){
                        tp[i]=cur;
                        if(cur.val+eps>r) flag=true;
                    }
                }
            }
            step*=0.55;
